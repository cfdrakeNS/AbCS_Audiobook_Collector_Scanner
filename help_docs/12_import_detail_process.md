# Import Detail Process

## What this is

The **Import Detail** window lets you review and fix one held import item at a time. Books land here when import finds duplicates, validation issues, fallback guesses, or other problems that need your attention.

## When to use it

- A row in the Import review table needs editing before you add it.
- You want to step through held items with Previous and Next.
- You need to save corrections, skip an item, or discard it.

## Before you start

- Run a folder import and open Import when books are held for review.
- In the Import review table, select a row and press **Enter** to open detail.

## Steps

1. In the Import window review table, select a held book.
2. Press **Enter** to open **Import Detail**.
3. Review validation errors and edit fields as needed.
4. Click **Save** (**Alt+S**) to keep your changes and return to the review list.
5. Click **Discard** (**Alt+D**) to drop this item without adding it.
6. Use **Page Up** and **Page Down** to move to the previous or next held item.
7. Press **Escape** to close detail and return to Import.

## What happens next

- Saved changes update the row in the Import review table.
- Use **Add Selected** (Alt+S) in Import to add approved rows to your library.
- Discarded items are removed from the review list.

## Related help

- Full folder import workflow: see [Import (Folder Scan)](02_import_process.md).
- Import settings and scenarios: see [Import Preferences](18_import_preferences.md).

## Shortcuts and accessibility

| Shortcut | Action |
|----------|--------|
| Alt+T | Title |
| Alt+A | Author |
| Alt+P | Plot |
| Alt+Y | Year |
| Alt+M | Time |
| Alt+R | Reader |
| Alt+I | Series |
| Alt+G | Genre |
| Alt+C | Collection |
| Alt+F | Files |
| Alt+B | Bitrate |
| Alt+Z | Size |
| Alt+E | Errors |
| Alt+H | Path |
| Alt+S | Save |
| Alt+D | Discard |
| Page Up | Previous item |
| Page Down | Next item |
| Shift+F1 | Help for this window |
| F1 | Keyboard shortcuts |
| Alt+/ | Re-read status |
| Escape | Close detail |

## Common confusion

**Save vs Add Selected — what is the difference?**
**Save** in Import Detail stores edits for one held item. **Add Selected** in the Import window adds checked rows to your library.

**Discard vs skipping in Import**
**Discard** in detail removes the current held item. You can also leave items in the review list and close Import; you will be asked to confirm if unscanned items remain.
