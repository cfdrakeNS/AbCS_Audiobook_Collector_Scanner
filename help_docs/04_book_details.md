...
3. The **Book Details** window shows all fields for that book. When no screen reader is running, a panel at the top also shows the book title, author, and series in larger text; with JAWS or NVDA running this panel is hidden and never spoken. If the audio file has a picture inside it, that picture shows at the top and is skipped when you press Tab.
