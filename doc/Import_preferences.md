...
This guide explains how **Preferences** shape folder import behavior: import scenarios, fallback rules, validation, and duplicate matching. It complements [Default preferences](abCS_default_preference.md) (factory values) and [Import process](abcs_import_process.md) (step-by-step workflow).
...
Choose the scenario that matches how your audiobook folders are organized. The scenario is saved in Preferences and applied when you scan from the Import window.
...
| **Mass Import - Series From Directory** | Series grouped in folders | Root → author → **series folder** → audio files |
...
- Treats the **book folder name** as the series name when the path looks like `Author/Series/Files`.
...
| **Title fallback to file** | Title tag missing or placeholder | Uses file name (strips leading track numbers) |
...
- Default values after **Restore Defaults** (Alt+R) are listed in [Default preferences](abCS_default_preference.md).

## Related documentation

- [Default preferences](abCS_default_preference.md) — factory default values
- [Import process](abcs_import_process.md) — folder scan workflow
- [Keyboard shortcuts by window](abcs_shortcuts_list.md) — Import and Preferences shortcuts
