...
Phase 1 put HTTP on a `QThread` but still **blocks** the main window with `popup.exec()`. This phase removes that block for Alt+W and Alt+B.
...
- **One job at a time.** A second Alt+W / Alt+B announces that a fetch is in progress and **Alt+J to return**; it does not start another thread.
...
1. Alt+W and Alt+B: can use the main list while progress is open.
