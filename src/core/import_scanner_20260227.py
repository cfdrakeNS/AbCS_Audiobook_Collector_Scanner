...
class ImportScanner:
    ...
    def log_import_details(self, book: Dict):
        log_path = os.path.join(os.getcwd(), "import_fallback_log.txt")
        with open(log_path, "a", encoding="utf-8") as log:
            log.write(f"Imported: {book.get('files', [''])[0]}\n")
            log.write(
                f"  Author: {book.get('author', '')} (Fallback: {book.get('_author_is_fallback', False)})\n")
            log.write(
                f"  Title: {book.get('title', '')} (Fallback: {book.get('_title_is_fallback', False)})\n")
            log.write(f"  Errors: {book.get('errors', [])}\n")
            log.write("---\n")
    ...
    def apply_preferences(self, book: Dict):
        ...
        self.log_import_details(book)
        errors = book.get("errors", [])

        # Apply fallback logic per field independently
        author = (book.get("author") or "").strip()
        title = (book.get("title") or "").strip()

        # Only apply fallback if not 'none' and author/title is missing
        if self.author_fallback_mode != "none" and self._is_placeholder_author(author):
            if self.author_fallback_mode == "folder" and folder:
                ...
                if fallback_author:
                    ...
                    book["_author_is_fallback"] = True
                    self._append_flag_once(
                        book,
                        "F: Author fallback from folder used",
                    )
            else:
                book["_author_is_fallback"] = False
        # If fallback is 'none', do not modify author

        if self.title_fallback_mode != "none" and self._is_placeholder_title(title):
            if self.title_fallback_mode == "folder" and folder:
                fallback_title = os.path.basename(folder.rstrip("\\/"))
                if fallback_title:
                    import re
                    fallback_title = re.sub(r'^\d+\s*', '', fallback_title)
                    book["title"] = fallback_title
                    fallback_applied.add("Title")
                    book["_title_is_fallback"] = True
                    self._append_flag_once(
                        book,
                        "F: Title fallback from folder used",
                    )
            else:
                book["_title_is_fallback"] = False
        # If fallback is 'none', do not modify title

        # Track fallback usage for duplicate logic
        book["_author_is_fallback"] = any(
            isinstance(err, str) and (
                err.startswith("F: Author fallback from folder used") or
                err.startswith("F: Author fallback from previous file used")
            ) for err in errors
        )
        book["_title_is_fallback"] = any(
            isinstance(err, str) and (
                err.startswith("F: Title fallback from folder used") or
                err.startswith("F: Title fallback from previous file used")
            ) for err in errors
        )
        ...
        if self._is_placeholder_title(title):
            ...
            # If folder fallback not used or empty, try file fallback
            if (not fallback_title or self._is_placeholder_title(fallback_title)) and self.title_fallback_mode == "file" and files:
            ...
            # If still no title, fallback to previous file (if error present)
            if (not fallback_title or self._is_placeholder_title(fallback_title)):
                for err in errors:
                    if isinstance(err, str) and err.startswith("F: Title fallback from previous file used"):
                        # Use previous file fallback, strip leading numbers
                        prev_title = book.get("title", "")
                        import re
                        prev_title = re.sub(r'^\d+\s*', '', prev_title)
                        if prev_title:
                            book["title"] = prev_title
                            fallback_applied.add("Title")
                            book["_title_is_fallback"] = True
                        break
        ...
        self._normalize_corrupted_read_error_messages(book)

    @staticmethod
    def _normalize_corrupted_read_error_messages(book: Dict):
        """Show file name for corrupted-read errors only when final title is empty."""
        ...
        files = book.get("files") or []
        fallback_file = os.path.basename(files[0]) if files else ""
        ...
        for err in errors:
            ...
            if "corrupted file or tags:" in lowered:
                if has_title:
                    text = "Corrupted file or tags:"
                else:
                    prefix_match = re.match(
                        r"^([^:]+):\s*corrupted file or tags:\s*$", text, re.IGNORECASE)
                    if prefix_match:
                        file_name = prefix_match.group(1).strip()
                        text = f"{file_name}: Corrupted file or tags:"
                    elif fallback_file:
                        text = f"{fallback_file}: Corrupted file or tags:"
                    else:
                        text = "Corrupted file or tags:"
