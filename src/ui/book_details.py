...
class BookDetailsWindow(AccessibleDialog):
    ...
    def apply_control_styles(self):
        ...
        header_labels = (
            getattr(self, "header_title_label", None),
            getattr(self, "header_meta_label", None),
        )
        for widget in self.findChildren(QLabel):
            if widget in header_labels:
                continue
            widget.setStyleSheet(label_style)
        self._apply_header_card_styles()
    ...
    def _update_header_card(self):
        """Refresh the decorative header card from the displayed field values."""
        if not hasattr(self, "header_card"):
            return
        title = self.title_edit.text().strip()
        if not title and self.is_new:
            title = "New Book"
        self.header_title_label.setText(title)
        parts = [
            text
            for text in (
                self.author_label_display.text().strip(),
                self.series_label_display.text().strip(),
            )
            if text
        ]
        self.header_meta_label.setText("  \u2022  ".join(parts))
        self.header_meta_label.setVisible(bool(parts))

    def _apply_header_card_styles(self):
        """Card panel plus emphasized title font; palette-based for themes."""
        if not hasattr(self, "header_card"):
            return
        self.header_card.setStyleSheet(build_card_panel_style("bookHeaderCard"))
        title_pt = max(self.scaler.get_scaled_size(13), 10)
        self.header_title_label.setStyleSheet(f"""
            QLabel#bookHeaderTitle {{
                font-weight: bold;
    ...
    def setup_ui(self):
        ...
        # latter turns on for any UIA client (touch keyboard, magnifier...)
        # and would hide the card even with JAWS/NVDA closed.
        from src.accessibility.screen_reader import is_screen_reader_active
        self.header_card = QWidget()
        self.header_card.setObjectName("bookHeaderCard")
        self.header_card.setFocusPolicy(Qt.NoFocus)
        header_card_layout = QVBoxLayout(self.header_card)
        header_card_layout.setContentsMargins(12, 8, 12, 8)
        header_card_layout.setSpacing(2)
        self.header_title_label = QLabel("")
        self.header_title_label.setObjectName("bookHeaderTitle")
        self.header_title_label.setFocusPolicy(Qt.NoFocus)
        self.header_title_label.setAccessibleName("")
        self.header_title_label.setAccessibleDescription("")
        self.header_title_label.setWordWrap(True)
        self.header_meta_label = QLabel("")
        self.header_meta_label.setObjectName("bookHeaderMeta")
        self.header_meta_label.setFocusPolicy(Qt.NoFocus)
        self.header_meta_label.setAccessibleName("")
        self.header_meta_label.setAccessibleDescription("")
        self.header_meta_label.setWordWrap(True)
        header_card_layout.addWidget(self.header_title_label)
        header_card_layout.addWidget(self.header_meta_label)
