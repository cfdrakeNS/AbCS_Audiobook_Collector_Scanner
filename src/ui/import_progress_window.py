...
class ImportProgressWindow(QDialog):
    ...
    Uses a real Qt parent (ImportWindow) so z-order stays above Import without
    ``WindowStaysOnTopHint`` (which floated over other applications). This
    matches the pre-AccessibleDialog behavior from before the Qt 6.11 polish.
    """
    ...
    def __init__(self, scaler: UIScaler, theme_manager: ThemeManager, parent=None):
        ...
        self.resize(920, 176)  # Widened so the full counters status line fits
    ...
    def setup_shortcuts(self):
        ...
        # WindowShortcut: ImportWindow forwards Alt+/ while it owns modal focus.
        self.status_shortcut.setContext(Qt.WindowShortcut)
        self.status_shortcut.activated.connect(self.on_read_status_bar)

        self.status_shortcut_shift = QShortcut(QKeySequence("Alt+?"), self)
        self.status_shortcut_shift.setContext(Qt.WindowShortcut)
        self.status_shortcut_shift.activated.connect(self.on_read_status_bar)

        self.escape_shortcut = QShortcut(QKeySequence(Qt.Key_Escape), self)
        # WindowShortcut is enough when this dialog is active; ImportWindow also
        # forwards Esc while it is the application-modal owner.
