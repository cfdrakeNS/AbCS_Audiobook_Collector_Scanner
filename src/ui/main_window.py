...
class MainWindow(QMainWindow):
    def show_read_date_dialog(self, row: int):
        ...
        # Set font size and calendar styling
        font = date_field.font()
        font.setPointSize(self.scaler.get_scaled_size(14))
        date_field.setFont(font)
        
        # Make calendar popup larger and more readable
        # Use 14pt base font, scale only if scaling is active
        base_font_size = 14
        if self.scaler.current_scale != 1.0:
            font_size = self.scaler.get_scaled_size(base_font_size)
        else:
        ...
        QCalendarWidget QToolButton {{
            font-size: {font_size}pt;
            font-weight: bold;
            min-height: {self.scaler.get_scaled_size(32)}px;
            min-width: {self.scaler.get_scaled_size(32)}px;
            padding: {self.scaler.get_scaled_size(6)}px;
        }}
        ...
        QCalendarWidget QAbstractItemView::item {{
            min-height: {self.scaler.get_scaled_size(35)}px;
            min-width: {self.scaler.get_scaled_size(35)}px;
            padding: {self.scaler.get_scaled_size(6)}px;
            font-weight: bold;
        }}
        QCalendarWidget QWidget#qt_calendar_navigationbar {{
            background-color: palette(window);
            min-height: {self.scaler.get_scaled_size(40)}px;
            font-weight: bold;
        }}
        QCalendarWidget QSpinBox {{
            ...
            font-weight: bold;
            min-height: {self.scaler.get_scaled_size(32)}px;
            min-width: {self.scaler.get_scaled_size(60)}px;
        }}
        """
        date_field.setStyleSheet(calendar_style)
...
# Resize columns - mw#22: Author, Title, Series, Genre stretch proportionally
    ...
    def create_menu_bar(self):
        ...
        # Removed setShortcut("Alt+W") to avoid conflict with shortcut manager
    ...
    def get_default_status(self) -> str:
        ...
        if self.duplicate_mode_active:
            return (
                ...
                "Use normal selection. Alt+D Delete, Escape Exit Dup Mode"
            )
    ...
    def _selection_shortcuts_text(self) -> str:
        ...
        if self.duplicate_mode_active:
            return "Alt+D Delete, Escape Exit Dup Mode"
    ...
    def _refresh_duplicate_mode_after_data_change(self):
        ...
        # Clear selection after delete - don't auto-select remaining duplicates
    ...
    def eventFilter(self, source, event):
        """Filter events for find-dialog Alt-key handling, combo anti-noise, and table mouse behavior."""
        if (
...
Use Ctrl+I to import or Alt+7 for menu options."""
