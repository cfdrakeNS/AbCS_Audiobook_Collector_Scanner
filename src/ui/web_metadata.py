...
class WebMetadataWindow(ProvenWebMetadataWindow):
    ...
    def __init__(self, db, book, scaler, theme_manager, parent=None, refresh_callback=None):
        # Only pass supported args to the proven base class
    ...
    def update_fields_with_web_data(self, web_data):
        """Update UI with accessible, text-based summary of changes and checkboxes."""
        ...
        def add_change_row(field_name, db_value, web_value, shortcut):
            ...
            if db_value is not None:
                db_label = QLabel(f"Current: {db_value}")
                db_label.setAccessibleName(f"Current {field_name}")
                row_layout.addWidget(db_label)
            arrow = QLabel("→")
            arrow.setAccessibleName("to")
            row_layout.addWidget(arrow)
            web_label = QLabel(f"Web: {web_value}")
            web_label.setAccessibleName(f"Web {field_name}")
            row_layout.addWidget(web_label)
            self.change_rows[field_name.lower()] = (row_widget, None)
            self.changes_layout.addWidget(row_widget)

        # Title
        if web_data.get('title') and (self.book.title is None or web_data['title'].strip().lower() != (self.book.title or '').strip().lower()):
            add_change_row("Title", self.book.title, web_data['title'], Qt.ALT + Qt.Key_T)
            self.field_differences['title'] = web_data['title']
        # Author
        if web_data.get('author') and (self.book.author_name is None or web_data['author'].strip().lower() != (self.book.author_name or '').strip().lower()):
            add_change_row("Author", self.book.author_name, web_data['author'], Qt.ALT + Qt.Key_A)
            self.field_differences['author'] = web_data['author']
        # Year
        if web_data.get('year'):
            db_year = str(self.book.year) if self.book.year else None
            if db_year is None:
                add_change_row("Year", None, str(web_data['year']), Qt.ALT + Qt.Key_Y)
                self.field_differences['year'] = str(web_data['year'])
            elif str(web_data['year']) != db_year:
                add_change_row("Year", db_year, str(web_data['year']), Qt.ALT + Qt.Key_Y)
                self.field_differences['year'] = str(web_data['year'])
        # Series
        if web_data.get('series'):
            db_series = self.book.series_name or None
            if db_series is None:
                add_change_row("Series", None, web_data['series'], Qt.ALT + Qt.Key_I)
                self.field_differences['series'] = web_data['series']
            elif web_data['series'].strip().lower() != db_series.strip().lower():
                add_change_row("Series", db_series, web_data['series'], Qt.ALT + Qt.Key_I)
                self.field_differences['series'] = web_data['series']
        # Genre
        if web_data.get('genre'):
            db_genre = self.book.genre_name or None
            if db_genre is None:
                add_change_row("Genre", None, web_data['genre'], Qt.ALT + Qt.Key_G)
                self.field_differences['genre'] = web_data['genre']
            elif web_data['genre'].strip().lower() != db_genre.strip().lower():
                add_change_row("Genre", db_genre, web_data['genre'], Qt.ALT + Qt.Key_G)
                self.field_differences['genre'] = web_data['genre']
        # Plot (always show if present)
        if web_data.get('plot') and web_data['plot'].strip() != (self.book.comments or '').strip():
            add_change_row("Plot", self.book.comments, web_data['plot'], Qt.ALT + Qt.Key_P)
            self.field_differences['plot'] = web_data['plot']
        # Announce number of changes
        num_changes = len(self.field_differences)
        self.set_status(f"{num_changes} field(s) differ. Use Alt+letter to focus checkboxes.")
