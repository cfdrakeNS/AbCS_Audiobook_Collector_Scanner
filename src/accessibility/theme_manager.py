...
from .style_helpers import (
    build_accessible_checkbox_style,
...
class ThemeManager(QObject):
    ...
    @staticmethod
    def _derive_mid_color(palette: QPalette) -> QColor:
        """
        Blend Base toward Text to get a border/gridline color that stays
        visible on both light and dark themes (roughly 35% text, 65% base).
        """
        base = palette.color(QPalette.Base)
        text = palette.color(QPalette.Text)
        ratio = 0.35
        return QColor(
            round(base.red() + (text.red() - base.red()) * ratio),
            round(base.green() + (text.green() - base.green()) * ratio),
            round(base.blue() + (text.blue() - base.blue()) * ratio),
        )
    ...
    def _apply_theme(self):
        ...
        # Derive the Mid role from the theme's own base/text colors.
        # Themes only set roles like Base and Text, so Mid would otherwise be
        # left over from the system palette and can be nearly invisible
        # against dark backgrounds. palette(mid) drives table gridlines,
        # cell borders, and header borders throughout the app.
        palette.setColor(QPalette.Mid, self._derive_mid_color(palette))

        # Apply to application
        self.app.setPalette(palette)

        checkbox_style = build_accessible_checkbox_style(
            palette.color(QPalette.Highlight).name(),
        )

        # Disable mouse hover highlighting for all tables.
        # Note: "QTableView:focus { border: none; }" here is only the
        # app-level fallback for tables that do NOT use
        # build_table_polish_style (style_helpers.py). Polished tables set a
        # widget-level stylesheet with a 2px highlight focus ring, which
        # takes precedence over this rule. Not a conflict — layered defaults.
        table_hover_disable = """
        ...
        if sys.platform.startswith("linux"):
            linux_sheet = build_linux_theme_stylesheet(
                table_hover_disable, scale_block
            )
            self.app.setStyleSheet(linux_sheet + "\n" + checkbox_style)
            return
