...
class ReadingQueries:
    ...
    def get_reading_statistics(
        ...
        stats = {
            ...
            "total_hours": (
                float(result["total_hours"]) if result["total_hours"] else 0.0
            ),
            "avg_hours_per_book": (
                float(result["avg_hours_per_book"])
                if result["avg_hours_per_book"]
                else 0.0
            ),
        }
        ...
        stats["most_productive_month"] = self._get_most_productive_month(
            start_date, end_date, collection_id
        )

        # Get yearly and monthly breakdowns
        stats["yearly_breakdown"] = self._get_yearly_breakdown(
            start_date, end_date, collection_id
        )
    ...
    def _get_most_productive_month(
        self,
        ...
        query += (
            " GROUP BY strftime('%Y-%m', read_date) ORDER BY book_count DESC LIMIT 1"
        )
    ...
    def _get_yearly_breakdown(
        ...
        query += (
            " GROUP BY strftime('%Y', read_date) ORDER BY year DESC"  # Show all years
        )
